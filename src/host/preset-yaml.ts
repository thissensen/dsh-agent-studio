...
import type { PresetPluginRow } from './types.js'
...
const JsExpr = new Type('tag:yaml.org,2002:js', {
    ...
    represent: (值) => (值 as { __jsExpr: string }).__jsExpr,
})
 ...
 * @returns row 数组。
 */
export function parsePlugins(文本: string, 说明: string): PresetPluginRow[] {
    if (文本.trim() === '') throw new Error(`${说明}的构成声明是空的`)
    ...
    try {
        ...
        throw new Error(`解析${说明}的构成声明失败：${(err as { message?: string }).message ?? String(err)}`)
    }

    if (!Array.isArray(数据)) throw new Error(`${说明}的构成声明不是一份 row 列表`)

    return 数据 as PresetPluginRow[]
}
 ...
 */
export function stringifyPlugins(行组: readonly PresetPluginRow[]): string {
