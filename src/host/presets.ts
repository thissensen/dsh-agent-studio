...
/** 设置服务本体（删除预设时按路径清配置分区要用它的 `mutate`）。 */
interface SettingsServiceLike {
    mutate?(namespace: string, ops: unknown[]): Promise<void>
}
...
interface PresetDeps {
    ...
    getSettings?(): SettingsServiceLike | undefined
    trustedHosts: unknown[]
}

/** `agentPresets` 服务的形状（只取 `list` / `copy` / `remove` / `authorable`）。 */
interface PresetService {
    list(): Promise<PresetListItem[]>
    copy?(sourceId: string, newId: string, displayName?: string): Promise<unknown>
    remove?(presetId: string): Promise<unknown>
    authorable?: boolean
}
 ...
 */
function readPersonaFlags(path: unknown): { promptFixed: boolean; snapshotFixed: boolean } {
    const off = { promptFixed: false, snapshotFixed: false }
    if (typeof path !== 'string') return off
 ...
 * @returns 供面板直接渲染的行数组。
 */
export async function listPresets(service: PresetService | undefined, createdIds: string[]): Promise<PresetRow[]> {
...
export async function handleListPresets(res: ServerResponse, deps: PresetDeps): Promise<void> {
    ...
    if (service?.list !== undefined) {
        ...
        try {
            ...
            json(res, 500, { ok: false, error: `读取预设清单失败：${(err as { message?: string } | undefined)?.message ?? String(err)}` })
            return
        }
    }
 ...
 */
async function writePresetMetadata(service: PresetService, presetId: string, name: string | undefined, description: string | undefined): Promise<void> {
    const rows = await service.list()
    const dir = rows.find((row) => row?.id === presetId)?.path
    if (typeof dir !== 'string' || dir === '') throw new Error('在预设清单里找不到刚建好的目录')

    const file = join(dir, 'preset.yml')
    const lines: string[] = []
    if (name !== undefined && name !== '') lines.push(`name: ${yamlScalar(name)}`)
    if (description !== undefined && description !== '') lines.push(`description: ${yamlScalar(description)}`)
...
export async function handleCreatePreset(req: IncomingMessage, res: ServerResponse, deps: PresetDeps): Promise<void> {
    ...
    try {
        ...
        json(res, 400, { ok: false, error: `新建预设失败：${(err as { message?: string } | undefined)?.message ?? String(err)}` })
        return
    }
    ...
    if (description !== undefined) {
        try {
            ...
            warning = `预设已创建，但描述没写上：${(err as { message?: string } | undefined)?.message ?? String(err)}`
        }
    }
    ...
    if (scope?.update !== undefined) {
        ...
        try {
            ...
            warning ??= `预设已创建，但来源标记没记住：${(err as { message?: string } | undefined)?.message ?? String(err)}`
        }
    }
 ...
 */
async function clearPresetTraces(deps: PresetDeps, presetId: string): Promise<string | undefined> {
    const scope = deps.getScope()
    ...
    if (nextBindings.length !== bindings.length || nextCreated.length !== created.length) {
        if (scope?.update === undefined) {
            ...
            try {
                ...
                warning = `预设已删除，但配置里的引用没清掉：${(err as { message?: string } | undefined)?.message ?? String(err)}`
            }
        }
    }
    ...
    if (settings?.mutate !== undefined) {
        try {
            ...
            warning ??= `预设已删除，但它的配置分区没清掉：${(err as { message?: string } | undefined)?.message ?? String(err)}`
        }
    }
...
export async function handleDeletePreset(req: IncomingMessage, res: ServerResponse, deps: PresetDeps): Promise<void> {
    ...
    try {
        ...
        json(res, 400, { ok: false, error: `删除预设失败：${(err as { message?: string } | undefined)?.message ?? String(err)}` })
        return
    }
