...
import type { Ctx, Logger, SettingsScope, SettingsService, StudioConfig } from './types.js'
...
export function apply(ctx: Ctx, config: unknown) {
    ...
    const registry = createPresetRegistry(
        ...
        { getConfig: () => (settingsScope === null ? undefined : settingsScope.get() as StudioConfig), logger },
    )
    ...
    ctx.inject(['agentPresets'], () => {
        void registry.sync().catch((err: unknown) => logger.error?.('[agent-studio] 自建预设重放失败', err))
    })
    ...
    ctx.inject(['settings'], (settingsCtx: Ctx) => {
        ...
        const scope: SettingsScope = {
            ...
            update: (patch: unknown) => settings.update(SETTINGS_NAMESPACE, patch as object),
        }
        ...
        ctx.on('settings/document-updated', (ns: unknown) => {
            ...
            void registry.sync().catch((err: unknown) => logger.error?.('[agent-studio] 自建预设同步失败', err))
        })
