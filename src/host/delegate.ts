...
/** 交给 `ctx.subagents.start()` 的请求对象（只含启动时能定的字段）。 */
...
/** 宿主 `subagents` 服务（派活层只用 `start` / `startContinuable` / `getProvider`）。 */
interface SubagentsService {
    start(provider: string, request: StartRequest): Promise<SubagentRun>
    startContinuable(options: { provider: string; label: string | undefined; request: StartRequest; signal: unknown; childId: string }): Promise<{ childId: string }>
    getProvider?(provider: string): unknown
}

/** `subagents.start` 返回的「运行」句柄。 */
...
/** `ToolExec` 在本插件用到的字段（`agent` / `signal`）。 */
interface StudioToolExec {
    agent?: unknown
    signal?: unknown
}

/** `studio_delegate` 的入参形状。 */
 ...
 */
const agentByChildSession = new Map<string, string>()
 ...
 */
export function agentOfChildSession(sessionId: string): string | undefined {
 ...
 * @param agentId - 代理 id。
 */
export function noteChildAgent(sessionId: string, agentId: string): void {
    if (agentByChildSession.has(sessionId)) agentByChildSession.delete(sessionId)
    ...
    while (agentByChildSession.size > MAX_TRACKED_CHILDREN) {
        ...
        agentByChildSession.delete(oldest as string)
    }
}
 ...
 */
export function claimChildAgent({ childId, parentSessionId }: { childId: string; parentSessionId: string }): string | undefined {
 ...
 * @returns 队列项，供失败时移除。
 */
function enqueueClaim(parentSessionId: string, agentId: string): Claim {
    const claim: Claim = { parentSessionId, agentId }
    pendingClaims.push(claim)

    while (pendingClaims.length > MAX_PENDING_CLAIMS) pendingClaims.shift()

    return claim
}

/** 移除一个队列项（start 失败、或精确路径已用不上的时候）。 */
function dropClaim(claim: Claim): void {
    const index = pendingClaims.indexOf(claim)
    if (index !== -1) pendingClaims.splice(index, 1)
}
 ...
 */
function subagentsOf(config: StudioConfig | undefined, agentId: string | undefined): AgentConfig[] {
    const owner = agentById(config, agentId)
    if (owner === undefined) return []

    const children: AgentConfig[] = []
 ...
 */
function ownerAgentIdFor(ctx: Ctx, settingsScope: SettingsScope, agent: unknown): string | undefined {
    const a = agent as ModelAgent | undefined
    const session = a?.session
    const sessionId = typeof session?.id === 'string' ? (session.id as string) : undefined
 ...
 */
function describeSubagent(agentConfig: AgentConfig): string {
 ...
 */
function startRequestFor(agentConfig: AgentConfig | undefined, label: string | undefined, parent: unknown, signal: unknown): StartRequest {
    const model = agentConfig?.model ?? {}
 ...
 * @param settingsScope - 本插件设置命名空间的 scope。
 */
export function mountDelegation(ctx: Ctx, logger: Logger, settingsScope: SettingsScope): void {
    const tools = ctx.get?.('tools') as { register(tool: unknown): unknown } | undefined
    ...
    tools.register(defineTool({
        ...
        execute(_args: unknown, exec: unknown) {
            ...
            if (lines.length === 0) return `代理「${ownerId}」名下还没有可派的子代理。`
    ...
    tools.register(defineTool({
        ...
        output: {
            ...
            render: (_args: unknown, value: unknown) => [{
                type: 'text',
                text: (value as { kind?: string; subagentId?: string }).kind === 'continuable'
                    ? `已启动子代理 ${(value as { subagentId?: string }).subagentId}（后台可续聊，等它结算或用 send_message 追问）`
                    : outputText((value as { output?: unknown }).output),
            }],
        },
        ...
        async execute(args: unknown, exec: unknown) {
            ...
            if (subagents?.start === undefined) throw Error('子代理服务未就绪，暂时不能派活。')
            ...
            const request = startRequestFor(targetConfig, label, callAgent, (exec as StudioToolExec).signal)
            ...
            const sessionId = callAgent.session?.id as string

            if (runInBackground) {
                // 可续子代理支持调用方指定会话 id ⇒ 映射在创建**之前**就写好，零竞态。
                // 队列项是保险（万一这个版本没采纳指定的 id）：返回后清掉。
                const childId = randomUUID()
                const claim = enqueueClaim(sessionId, targetConfig.id as string)
                noteChildAgent(childId, targetConfig.id as string)
                ...
                try {
                    const started = await subagents.startContinuable({
                        ...
                        label,
                        request,
                        signal: (exec as StudioToolExec).signal,
                        childId,
                    })
                    dropClaim(claim)
            ...
            noteChildAgent(run.id, targetConfig.id as string)
            dropClaim(claim)
...
function outputText(blocks: unknown): string {
    if (!Array.isArray(blocks)) return '（子代理没有返回内容）'

    const texts = (blocks as Array<{ type?: string; text?: unknown }>)
