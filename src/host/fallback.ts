...
import type { AgentConfig, Ctx, Logger, ModelConfig, SettingsScope } from './types.js'
...
const DEFAULT_RETRYABLE_CODES: readonly string[] = ['EMPTY_RESPONSE', 'RATE_LIMIT', 'SERVER', 'TIMEOUT', 'TRANSPORT']
...
/** 本文件用到的 agent 成员（宿主对象；精确契约在平台源码，够用即可）。 */
interface ModelAgent {
    ctx?: unknown
    session: {
        id?: unknown
        header?: { parentSession?: unknown }
        append(event: string, data: unknown): unknown
    }
}

/** 一个候选路由（provider 与 model 都非空）。 */
...
/** 降级链：`routes[0]` 恒为 null——「不改写」档，保留平台/用户定下的路由。 */
interface FallbackChain {
    routes: (CompleteRoute | null)[]
    maxRetries: number
}

/** 一次失败的现场（`agent/request-error` payload 里本文件要用的字段）。 */
...
interface FallbackState {
    ...
    /** 当前候选上已经重试的次数。 */
    retryCount: number
    /** 上一次失败所属的 turn/step；换 step 即从零计。 */
    stepKey: string | undefined
    /** 我们上次实际交给平台的路由（改写后）——它变了说明用户手动换了模型。 */
    lastUsed: { provider: string; model: string } | undefined
    /** 待投递的切换通知（下一个 pre-step 时发给模型）。 */
    notice: SwitchNotice | undefined
}
 ...
 */
function buildChain(agentConfig: AgentConfig, isChild: boolean): FallbackChain | undefined {
    const fallbacks: CompleteRoute[] = []
    for (const row of agentConfig.fallbacks ?? []) {
        const route = completeRoute(row)
        if (route !== undefined) fallbacks.push(route)
    }

    if (fallbacks.length === 0) return undefined

    const routes: (CompleteRoute | null)[] = [null]
    if (isChild === false) {
        const head = completeRoute(agentConfig.model)
        if (head !== undefined) routes.push(head)
    }
...
function completeRoute(row: ModelConfig | undefined): CompleteRoute | undefined {
    if (row?.provider === undefined || row.model === undefined) return undefined
    if (row.provider === '' || row.model === '') return undefined

    const route: CompleteRoute = { provider: row.provider, model: row.model }
    if (row.reasoningEffort !== undefined && row.reasoningEffort !== '') route.reasoningEffort = row.reasoningEffort
...
function retryLimit(value: unknown): number {
    if (typeof value !== 'number' || Number.isSafeInteger(value) === false || value < 0) return DEFAULT_MAX_RETRIES

    return value
}

/** 这次请求的可重试码表：平台给的策略优先（provider 可定制），缺了用平台默认。 */
function retryableCodesOf(policy: unknown): readonly string[] {
    const codes = (policy as { retryableCodes?: unknown } | undefined)?.retryableCodes
 ...
 */
function policyKeyOf(policy: unknown): string {
    const p = policy as {
        mode?: unknown
        maxRetries?: unknown
...
function routeOf(config: unknown): { provider: string; model: string } | undefined {
    const c = config as { provider?: unknown; model?: unknown } | undefined
    if (c === undefined || typeof c.provider !== 'string' || typeof c.model !== 'string') return undefined
    if (c.provider === '' || c.model === '') return undefined
...
/** 两条路由是否相同。 */
function sameRoute(a: { provider: string; model: string }, b: { provider: string; model: string }): boolean {
    return a.provider === b.provider && a.model === b.model
}

/** 用候选路由改写这次请求的 config（继承的思考强度让位给候选自己的）。 */
function overrideRoute(resolved: unknown, candidate: CompleteRoute): unknown {
    const { reasoningEffort: _inherited, ...rest } = resolved as Record<string, unknown>
...
async function waitFor(delayMs: number, signal: AbortSignalLike | undefined): Promise<boolean> {
...
async function scheduleRetry(
    ...
    if (await waitFor(delayMs, fields.signal) === false) return undefined
 ...
 */
function switchNotice(notice: SwitchNotice): unknown {
 ...
 */
export function mountFallback(ctx: Ctx, logger: Logger, settingsScope: SettingsScope): void {
    const states = new WeakMap<object, FallbackState>()

    /** 取（或建）一个 agent 的降级状态。 */
    function stateOf(agent: object): FallbackState {
        const known = states.get(agent)
        if (known !== undefined) return known

        const created: FallbackState = {
            activeIndex: 0,
            retryCount: 0,
            stepKey: undefined,
            lastUsed: undefined,
     ...
     * 「预设 → 绑定」——与装配期同一条解析路径。
     */
    function chainFor(agent: ModelAgent): FallbackChain | undefined {
        ...
        if (isChild) {
            ...
            if (typeof sessionId === 'string') agentId = agentOfChildSession(sessionId)
     ...
     */
    function advance(state: FallbackState, chain: FallbackChain, code: string | undefined): boolean {
    ...
    async function rewriteRequest(payload: unknown, next: unknown): Promise<unknown> {
        const resolved = await (next as () => Promise<unknown>)()

        try {
            const agent = (payload as { agent?: ModelAgent } | undefined)?.agent
            if (agent === undefined) return resolved

            const chain = chainFor(agent)
            if (chain === undefined) return resolved
            ...
            if (candidate === undefined || candidate === null) {
                if (incoming !== undefined) state.lastUsed = incoming
     ...
     */
    async function decideRetry(payload: unknown, next: unknown): Promise<unknown> {
        const fields = payload as RequestFailure | undefined
        const agent = fields?.agent

        if (fields === undefined || agent === undefined) return await (next as () => Promise<unknown>)()
        if (fields.signal?.aborted === true) return undefined

        try {
            const chain = chainFor(agent)
            // 不接管 = 原样放行：内置重试（dsh-llm-retry）照常
            if (chain === undefined) return await (next as () => Promise<unknown>)()
    ...
    async function announceSwitch(payload: unknown, next: unknown): Promise<unknown> {
        const fields = payload as { agent?: ModelAgent; signal?: AbortSignalLike } | undefined
        const decision = await (next as () => Promise<unknown>)() as
            { kind?: string; messages?: unknown[] } | undefined

        const agent = fields?.agent
        if (agent === undefined) return decision

        const state = states.get(agent)
        if (state?.notice === undefined) return decision
        if (decision === undefined || decision.kind === 'reject' || fields?.signal?.aborted === true) return decision
