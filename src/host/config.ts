...
export const SectionSchema = z.object<SectionConfig>({
...
export const BACKGROUND_MODES: BackgroundMode[] = ['always-background', 'foreground', 'model-decides']

/** Agent 模式三档：跟随预设（默认）/ 原生 / 强制 PTC。 */
export const TOOL_PRESENTATIONS: ToolPresentation[] = ['follow', 'native', 'ptc']

/** 自动降级的默认重试上限：与平台默认策略（dsh-llm 的 `maxRetries`）对齐。 */
export const DEFAULT_MAX_RETRIES = 5

/** 一个代理。 */
const AgentSchema = z.object<AgentConfig>({
...
/** 提示词集。 */
const PromptSetSchema = z.object<PromptSetConfig>({
    id: z.string().required(),
    name: z.string().default(''),
    note: z.string().default(''),
...
/** 工具集。 */
const ToolSetSchema = z.object<ToolSetConfig>({
    id: z.string().required(),
    name: z.string().default(''),
    note: z.string().default(''),
...
const SkillSetSchema = z.object<SkillSetConfig>({
    id: z.string().required(),
    name: z.string().default(''),
    note: z.string().default(''),
 ...
 */
const PresetBindingSchema = z.object<PresetBinding>({
    presetId: z.string().required(),
    agentId: z.string().required(),
})

const BindingsSchema = z.object<BindingsConfig>({
    presets: z.array(PresetBindingSchema).default([]),
})

/** 顶层配置。 */
export const ConfigSchema = z.object<StudioConfig>({
