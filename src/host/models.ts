...
/** `handleListModels` / `handleListEfforts` 的依赖：延迟取 `llm` 服务。 */
interface ModelDeps {
    getLlm(): unknown
}

/** 宿主 `llm` 服务的形状（只取本项目用到的成员）。 */
 ...
 */
export async function handleListModels(res: ServerResponse, deps: ModelDeps): Promise<void> {
    const llm = deps.getLlm() as LlmService | undefined
    ...
    for (const provider of llm.listProviders()) {
        ...
        try {
            ...
            warnings.push(`${provider.id}（${(err as { message?: string } | undefined)?.message ?? String(err)}）`)
        }
    }
 ...
 * @param deps - `{ getLlm }`。
 */
export async function handleListEfforts(req: IncomingMessage, res: ServerResponse, deps: ModelDeps): Promise<void> {
    const llm = deps.getLlm() as LlmService | undefined
    ...
    try {
        const info = await llm.resolveModelInfo(provider, model) as ModelInfo | undefined
        ...
        json(res, 400, { ok: false, error: `解析模型档位失败：${(err as { message?: string } | undefined)?.message ?? String(err)}` })
    }
}
