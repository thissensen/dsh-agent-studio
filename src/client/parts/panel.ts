...
/**
 * 导入确认弹窗的目标：`'config'` = 全量导入（顶栏），其余是池级导入（底栏）的池名。
 *
 * 两种导入共用同一个弹窗与同一套合并规则，只在标题与「读哪个文件输入框」上分开。
 */
type ImportTarget = 'config' | PoolKind
...
interface PresetBarProps {
    ...
    /** 隐藏文件输入的 onChange：面板拿到文件后按弹窗里定下的规则合并。 */
...
interface SaveBarProps {
    ...
    onExportPool: () => void
    /** 池级导入按钮：打开导入确认弹窗。 */
    onRequestImportPool: () => void
    /** 隐藏文件输入的 onChange：面板拿到文件后按弹窗里定下的规则合并。 */
    onImportPoolFile: (event: ChangeEvent<HTMLInputElement>) => void
...
interface ImportModalProps {
    ...
    /** 池级导入时标题里的池名。 */
    poolWord: string
    /** 撞 id 时用导入项覆盖现有项；关闭 = 整批拒绝并报错。 */
    overwrite: boolean
    busy: boolean
    onToggleOverwrite: (next: boolean) => void
    onPickFile: () => void
    onCancel: () => void
}
...
function PresetBar(props: PresetBarProps) {
    ...
    const transferRow = h('div', { key: 'transferRow', style: { ...STYLE.row, width: '100%' } }, [
        ...
        // 隐藏的文件输入不能换成平台 `Input`：按钮先开确认弹窗，弹窗确认后由面板
        // 点开它（原来是 `<label htmlFor>`，换平台件后没有 label 那种转发）。
        h(Button, {
            key: 'import',
            type: 'button',
            variant: 'outline',
...
function SaveBar(props: SaveBarProps) {
    const { busy, dirty, savedOnce, poolWord, onSave, onExportPool, onRequestImportPool, onImportPoolFile, onHeightChange } = props
    const t = useT()
    ...
    return h('div', { ref: barRef, style: dirty === true ? SAVE_BAR_DIRTY : SAVE_BAR_CLEAN }, [
        ...
        h('input', {
            ...
            accept: 'application/json,.json',
            style: { display: 'none' },
            onChange: onImportPoolFile,
        }),
        h(Button, {
            key: 'poolImport',
            type: 'button',
            variant: 'outline',
            onClick: onRequestImportPool,
            disabled: busy,
...
/**
 * 导入确认弹窗：全量导入与池级导入共用（2026-09-20 用户拍板——不要再各写一套）。
 *
 * 只在**选文件之前**问一件事：撞 id 时覆盖还是报错。规则定下后由面板带进合并，
 * 合并时不再二次询问。两行说明是静态的，开关只决定走哪一条，所以不随状态换文案。
 *
 * @param props - 弹窗目标、开关值与三个回调。
 */
function ImportModal(props: ImportModalProps): ReactElement {
    const { target, poolWord, overwrite, busy, onToggleOverwrite, onPickFile, onCancel } = props
    const t = useT()

    const title = target === 'config' ? t('import.title') : t('import.poolTitle', { pool: poolWord })
    ...
    return h(Modal, {
        ...
        closeLabel: t('common.close'),
        footer: formButtons({
            okText: t('import.pickFile'),
            okDisabled: busy,
            onOk: onPickFile,
            onCancel,
        }, t),
    }, [
        h('div', { key: 'overwrite', style: STYLE.rowNowrap }, [
            h(Switch, {
                key: 'switch',
                checked: overwrite,
                onChange: (next: boolean) => onToggleOverwrite(next),
                // 平台的 label 是**无障碍名**（不可见）⇒ 可见文案另写一个 span。
                label: t('import.overwrite'),
            }),
            h('span', { key: 'caption', style: STYLE.hint }, t('import.overwrite')),
        ]),
        h('div', { key: 'on', style: STYLE.hint }, t('import.overwriteOn')),
        h('div', { key: 'off', style: STYLE.hint }, t('import.overwriteOff')),
    ])
}
...
function PanelBody() {
    ...
    /** 导入确认弹窗的目标：`null` = 不弹。全量与池级导入共用这一个弹窗。 */
    const [importAsk, setImportAsk] = useState<ImportTarget | null>(null)
    /** 弹窗里那个开关：撞 id 时用导入项覆盖现有项（关 = 整批拒绝并报错）。 */
    ...
    /**
     * 把一个池的导入项并进草稿——判 id，撞 id 时按 `overwrite` 决定**覆盖还是整批拒绝**
     * （拒绝是 2026-09-17 定下的老规矩；2026-09-20 用户加开关，默认仍是拒绝，覆盖要显式打开）。
     *
     * 覆盖 = 用导入项替换同 id 的现有项，**保持它原来的位置**（不追加到尾部，也不改名、
     * 不去动别处的引用）。合并结果先落在草稿里，用户看过再点保存。
     * Agent 额外做引用校验，`refs` 是（合并后的）三个集池的 id 集合。
     *
     ...
     * @param overwrite - 撞 id 时是否用导入项覆盖现有项。
     * @returns 合并后的新数组。
     */
    function mergePoolItems(
        kind: PoolKind,
        existing: DraftItem[],
        incoming: RawPoolItem[],
        refs: RefsOf | undefined,
        overwrite: boolean,
    ): DraftItem[] {
        ...
        for (const raw of incoming) {
            ...
            if (before !== undefined && overwrite !== true) {
    ...
    async function importPool(event: ChangeEvent<HTMLInputElement>) {
        ...
        try {
            ...
            setActionState({
                status: 'ok',
                message: t('validate.importPoolDone', { count: incoming.length, noun: nounOf(kind, t) }),
            })
     ...
     *
     * 内部就是按池**依次合并**（先集后代理——代理要引用集，反了会自己报缺集），
     * 规则与池级导入同一套：判 id，撞 id 时按弹窗里定下的 `importOverwrite` 覆盖或拒绝。
     */
    async function importConfig(event: ChangeEvent<HTMLInputElement>) {
        ...
        try {
            ...
            const incomingSkillSets = readImportItems(payload, 'skillSets')
            const incomingPromptSets = readImportItems(payload, 'promptSets')
            const incomingToolSets = readImportItems(payload, 'toolSets')
            const incomingAgents = readImportItems(payload, 'agent')

            const skillSets = mergePoolItems(
                'skillSets',
                draft!.skillSets,
                incomingSkillSets,
                undefined,
            ...
            const promptSets = mergePoolItems(
                'promptSets',
                draft!.promptSets,
                incomingPromptSets,
                undefined,
            ...
            setActionState({
                ...
                message: t('validate.importConfigDone', {
                    promptSets: incomingPromptSets.length,
                    toolSets: incomingToolSets.length,
                    skillSets: incomingSkillSets.length,
                    agents: incomingAgents.length,
                }),
            })
    ...
    /**
     * 弹窗确认后去点对应的隐藏文件输入框——弹窗只管规则，文件由那两个输入框收。
     *
     * 先关弹窗再点：文件选择器是模态的，留着弹窗在下面容易让人以为还能改开关。
     * 用哪个输入框由弹窗的目标决定（顶栏的全量 / 底栏的池级）。
     */
    function pickImportFile() {
        const inputId = importAsk === 'config' ? 'agent-studio-import' : 'agent-studio-import-pool'
        setImportAsk(null)
    ...
    // 导入确认弹窗：全量与池级共用，所以只挂这一个实例，认目标由 `importAsk` 负责。
    const importModal = importAsk === null ? null : h(ImportModal, {
        key: 'importModal',
        target: importAsk,
        poolWord,
        overwrite: importOverwrite,
        busy,
        onToggleOverwrite: (next: boolean) => setImportOverwrite(next),
    ...
    return h('div', { style: STYLE.panel }, [
        h(PresetBar, {
            ...
            onRequestImport: () => setImportAsk('config'),
            onImportFile: importConfig,
        }),
        ...
        h(SaveBar, {
            ...
            onRequestImportPool: () => setImportAsk(selection.kind),
            onImportPoolFile: importPool,
