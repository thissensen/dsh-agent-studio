...
export const zh = {
    ...
    'background.modelDecides': 'AI自行决定',
    ...
    'section.ptcRequired': 'PTC模式必备工具，仅PTC模式下生效',
    'section.ptcRequiredShort': 'PTC专属',
    ...
    'bar.bindHint': '给预设绑定后，将会覆盖Agent的所有提示词、工具、skill',
    'bar.unsaved': '有未保存的改动。',
    'bar.saved': '已保存。下一次装配起生效。',
    'bar.save': '保存改动',
    'bar.poolImport': '{pool}导入',
    'bar.poolExport': '{pool}导出',
    'bar.poolImportTitle': '导入{pool}（并进当前草稿的这个池；id 撞车会整批拒绝）',
    ...
    'preset.createNote': '「新建」= 把来源预设的整个目录复制一份（提示词组合、技能、资源都带上），源预设不动；描述留空 = 继承来源预设的。',
    'preset.deleteTitle': '删除预设',
    'preset.deleteConfirm': '删除预设「{name}」？预设文件会被平台删除，不可撤销。',
    ...
    // ── Agent 详情 ────────────────────────────────────────────────────────
    'agent.identity': '身份',
    'agent.notePlaceholder': '描述Agent用途，被当做子代理时，AI通过这个决定调用',
    'agent.promptSets': '提示词集',
    'agent.promptSetsHint': '按顺序拼接最终提示词，不填则为空',
    'agent.promptSetsFixed': 'complete:为true的无法设定提示词',
    ...
    'agent.toolSets': '工具集',
    'agent.toolSetsHint': '给Agent配备的工具',
    ...
    'set.skillHint': '给Skill分组管理，仅用于让AI识别调用的部分',
...
export const en: Record<CopyKey, string> = {
    ...
    'bar.poolImportTitle': 'Import {pool} (merged into this pool of the current draft; any id clash rejects the whole batch)',
    ...
    'preset.createNote': '"New" copies the source preset\'s whole directory (prompt composition, skills, resources), leaving the source untouched; an empty description inherits the source preset\'s.',
    'preset.deleteTitle': 'Delete preset',
    'preset.deleteConfirm': 'Delete the preset "{name}"? The platform deletes the preset files; this cannot be undone.',
