 ...
 */
const reachableByPreset = new Map();
function remember(sessionId, snapshot) {
 ...
 */
export function observeFor(sessionId) {
 ...
 */
function readToolSurfaces(ctx, scope) {
    try {
        const tools = ctx.get?.('tools');
        if (tools?.view === undefined)
            return undefined;
 ...
 */
export function skillRowOf(skill) {
 ...
 */
async function readSkillCatalog(ctx, context) {
    try {
 ...
 */
function ownersFor(names, ownerOf) {
 ...
 */
export function mountObservation(ctx, logger, deps = {}) {
    const ownerOf = deps.ownerOf;
    ctx.on('system-prompt/assemble', async (assemblyArg, contextArg, nextArg) => {
        const next = nextArg;
        const context = contextArg;
        const assembly = assemblyArg;
        ...
        if (!Array.isArray(sections) || sessionId === undefined)
            return result;
        ...
        if (presetId !== undefined && reachable !== undefined)
            reachableByPreset.set(presetId, reachable);
        ...
        rememberPresetObservation(presetId, { sections: sectionFacts, variables, tools: toolFacts, skills }, logger);
        // 只在该会话首次被观察时打，避免每个请求都刷屏。
        if (isFirstSight) {
            logger.info?.(`[agent-studio] 首次观察到会话 ${sessionId.slice(0, 18)}… · 预设 ${presetId ?? '解析不出'} · 段 ${sections.length} · 工具 ${surfaces?.visible?.length ?? '?'}`);
