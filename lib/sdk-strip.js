 ...
 */
export function stripSdkEntries(text, removeNames) {
 ...
 */
function scanSdkEntries(lines, removeNames) {
    const output = [];
    const removed = [];
    let index = 0;
    while (index < lines.length) {
        ...
        output.push(header);
        index += 1;
        if (!SDK_MAP_HEADER.test(header))
            continue;
        while (index < lines.length && lines[index] !== '}') {
            const unit = readEntryUnit(lines, index);
            if (unit === undefined)
                return undefined;
            if (removeNames.has(unit.key))
                removed.push(unit.key);
            else
                output.push(...unit.lines);
            index = unit.end;
        }
 ...
 */
function readEntryUnit(lines, start) {
    const unitLines = [];
    let index = start;
    ...
    while (index < lines.length) {
        ...
        if (next === '}' || SDK_DOC_LINE.test(next) || SDK_ENTRY_START.test(next))
            break;
        if (!SDK_CONTINUATION_LINE.test(next))
            return undefined;
        unitLines.push(next);
        index += 1;
    }
