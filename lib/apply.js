 ...
 */
const appliedPresentations = new WeakMap();
/**
 ...
 */
export function mountApply(ctx, logger, settingsScope) {
     ...
     * 这一行把判断依据一次给全，不用逐项试。
     */
    const reportFirstDecision = (facts) => {
     ...
     * 和「收窄成空」是两回事。
     */
    const handleChildCreated = (agent) => {
     ...
     */
    const rewrite = async (result, context) => {
        ...
        const snapshotOff = config?.runtimeContextOff?.includes(target?.agentId) === true;
        ...
        if (target.runtimeSurface && tools !== undefined) {
            if (applyToolSurface(logger, tools, agent, target.tools, target.label)) {
                refreshSdkSection(tools, context, result);
            }
        }
        ...
        if (schemas !== undefined)
            next = { ...next, tools: schemas };
        // SDK 声明正文（PTC 下模型唯一的工具知识来源）走的是另一条投递通道，上面那步
        // 管不到它；按同一判据再裁一道，让本层漏网工具连「能看」都不剩。
        let sdkRemovedNames = [];
        if (tools !== undefined) {
            sdkRemovedNames = stripSdkSectionText(tools, agent, target.tools, next, target.label);
        }
 ...
 */
function resolveTarget(config, agent, presetId) {
    ...
    if (agentId === undefined)
        agentId = boundAgentId(config, presetId);
    const agentConfig = agentById(config, agentId);
    if (agentConfig === undefined)
        return undefined;
    return {
 ...
 */
function toolPolicyOf(config, toolSetIds) {
 ...
 */
function skillPolicyOf(config, skillSetIds) {
 ...
 */
function buildSections(platformSections, sectionConfigs) {
 ...
 */
function mountChildToolSurface(logger, agent, toolNames, label) {
    ...
    if (typeof tools.guard === 'function') {
        disposers.push(tools.guard((exec) => {
            ...
            if (typeof name !== 'string')
                return undefined;
            if (MECHANISM_TOOL_NAMES.has(name) || wanted.has(name))
                return undefined;
 ...
 */
function applyToolSurface(logger, tools, agent, policy, label) {
    ...
    if (applied !== undefined) {
        for (const dispose of applied.disposers)
            dispose();
    ...
    const denyReason = (exec) => {
        const name = exec?.name;
        if (typeof name !== 'string' || !isDenied(name))
            return undefined;
    ...
    try {
        if (deny.length > 0)
            disposers.push(agentCtx.tools.restrict({ deny }));
 ...
 */
function restrictToolSchemas(schemas, policy) {
 ...
 * @param label - 这套面属于谁，用于日志。
 */
async function applySkillSurface(logger, agent, policy, label) {
    if (policy === undefined)
        return;
    const applied = appliedSkills.get(agent);
    const agentCtx = agent?.ctx;
    const skillService = (typeof agentCtx?.get === 'function' ? agentCtx.get('skills') : undefined);
    ...
    const hidden = listed
        ...
        .filter((name) => typeof name === 'string' && !include.has(name));
 ...
 * @returns 归一化后的三档值。
 */
function presentationOf(config, agentId) {
    const value = agentById(config, agentId)?.toolPresentation;
    if (value === 'native' || value === 'ptc')
        return value;
    return 'follow';
}
/**
 ...
 */
function applyToolPresentation(logger, ctx, agent, mode, label) {
 ...
 */
function applyMainPresentation(ctx, logger, config, agent, source) {
 ...
 */
function readRestrictableNames(tools, agent) {
    try {
 ...
 */
function refreshSdkSection(tools, context, result) {
 ...
 */
function stripSdkSectionText(tools, agent, policy, result, label) {
    const sdkSection = result?.sections?.find((section) => section?.name === 'tools:sdk');
    if (typeof sdkSection?.text !== 'string')
        return [];
    let visible;
    try {
    ...
    for (const name of visible.keys()) {
        if (MECHANISM_TOOL_NAMES.has(name) || keep.has(name))
            continue;
