 ...
 */
export function validateRequestOrigin(req, trustedHosts) {
    const host = typeof req.headers.host === 'string' ? req.headers.host : '';
    const hostUrl = parseAuthority(host);
    if (hostUrl === undefined)
        return forbiddenHost();
    if (!isCanonicalAuthority(host, hostUrl))
        return forbiddenHost();
 ...
 */
export function validateMutationRequest(req, trustedHosts) {
    ...
    return new Promise((resolve, reject) => {
        ...
        req.on('end', () => {
            ...
            try {
                ...
                fail(Object.assign(Error(`invalid JSON body: ${err.message}`), { statusCode: 400 }));
            }
        });
        req.on('error', fail);
    });
