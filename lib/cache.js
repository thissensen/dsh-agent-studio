 ...
 */
export function cachedObservations() {
    if (loaded !== null)
        return loaded;
    try {
        const parsed = JSON.parse(readFileSync(join(cacheDir(), 'observations.json'), 'utf8'));
 ...
 */
export function rememberPresetObservation(presetId, snapshot, logger) {
    if (presetId === undefined)
        return;
    const cached = cachedObservations();
    const previous = cached[presetId];
    const next = { presetId, ...snapshot };
    if (previous !== undefined && isSameSurface(previous, next))
        return;
    ...
    try {
        ...
        logger?.warn?.(`[agent-studio] 观察缓存没写进去（不影响本次装配）：${err?.message ?? String(err)}`);
    }
}
/**
 ...
 * @param right - 新快照。
 */
function isSameSurface(left, right) {
    const leftCopy = { ...left };
