 ...
 */
export function mountAttribution(ctx, logger) {
    /** 注册时刻记下的工具名 → 注册者名字。 */
    const live = new Map();
    /** 包装过的实例（幂等，别越包越深）。 */
    ...
    const selfName = ctx.fiber?.fiber?.name;
    /**
     * 读当前归属：reader 缺失、是 root、或就是本插件自己时给 undefined，
     * 交给静态兜底表——宁可不标，也不能把归属安错人。
     */
    const readerName = () => {
        const name = lastReader?.fiber?.name;
        if (typeof name !== 'string' || name === '' || name === 'root' || name === selfName)
            return undefined;
        return name;
    };
    /** 把实例的 `register` 包一层，把注册时刻的 reader 记进 live 表。 */
    const wrapInstance = (tools) => {
        if (tools === null || typeof tools !== 'object' || wrappedInstances.has(tools))
            return;
        const svc = tools;
        ...
        const wrappedRegister = function (definition) {
            ...
            if (owner !== undefined && typeof def?.name === 'string')
                live.set(def.name, owner);
