 ...
 */
async function readPersonaFlags(service, presetId) {
    const off = { promptFixed: false, snapshotFixed: false };
    if (service.readDocument === undefined)
        return off;
    let content;
    try {
 ...
 * @returns 供面板直接渲染的行数组。
 */
export async function listPresets(service, created) {
 ...
 */
function presetIdProblem(presetId, taken) {
    if (!PRESET_ID.test(presetId)) {
        return '新 id 只能用小写字母、数字、点、下划线或连字符，且不能以分隔符开头结尾';
    }
    if (taken.has(presetId))
        return `已经有一个叫「${presetId}」的预设了`;
    return undefined;
}
/**
 ...
 * @returns row 数组。
 */
async function readSourcePlugins(service, sourceId) {
    if (service.readDocument === undefined)
        throw new Error('平台的预设服务没有读构成声明的入口');
    const content = (await service.readDocument(sourceId))?.content;
    if (typeof content !== 'string')
        throw new Error(`来源预设「${sourceId}」的构成声明读不出来`);
 ...
 */
function bindingsWithout(config, presetId) {
...
export async function handleDeletePreset(req, res, deps) {
    ...
    const patch = { createdPresets: kept };
    if (bindings !== undefined)
        patch.bindings = { presets: bindings };
    try {
