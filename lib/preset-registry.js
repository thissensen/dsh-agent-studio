...
function toDeregister(deregister) {
...
function isRegisterable(id, definition) {
 ...
 * @returns 注册表。
 */
export function createPresetRegistry(service, deps) {
    /** 已注册的注销器，按预设 id 索引。 */
    const 注销器组 = new Map();
    /** 给注册调用套一层超时（平台卡住时不能把 HTTP 端点一起拖死）。 */
    async function 限时注册(定义) {
        let 计时器;
        const 超时 = new Promise((_resolve, reject) => {
            计时器 = setTimeout(() => reject(new Error(`注册超过 ${REGISTER_TIMEOUT_MS}ms 没返回`)), REGISTER_TIMEOUT_MS);
        });
        try {
            return await Promise.race([Promise.resolve(service()?.register?.(定义)), 超时]);
        }
        finally {
            clearTimeout(计时器);
        }
    ...
    async function 注册一份(定义, 预设id) {
        ...
        if (注销 === undefined)
            throw new Error('平台没有返回注销器，这个预设会删不掉');
        注销器组.set(预设id, 注销);
    }
    async function sync() {
        const 配置组 = deps.getConfig()?.createdPresets ?? [];
        const 想要 = new Set(配置组.filter((定义) => isRegisterable(定义?.id, 定义)).map((定义) => 定义.id));
        // ① 配置里已经没有的：注销。
        ...
        for (const 定义 of 配置组) {
            const 预设id = 定义?.id;
            if (typeof 预设id !== 'string' || 预设id === '' || 注销器组.has(预设id))
                continue;
    ...
    return {
        ...
        async create(定义, update) {
            const 预设id = 定义.id;
            if (typeof 预设id !== 'string' || 预设id === '')
                throw new Error('预设 id 不能为空');
            if (注销器组.has(预设id))
                throw new Error(`已经有一个叫「${预设id}」的自建预设了`);
            await 注册一份(定义, 预设id);
