...
- 本插件提供替代路径：`studio_list_subagents` 列出该 agent 名下的子代理，`studio_delegate` 按代理 id 派活——**只有走它，子代理那套独立配置（提示词 / 工具 / 技能 / 模型）才认得出身份、才生效**。官方 `subagent` 没有特殊待遇（工具集里留着它就在），但它派出的子代理没有角色登记，只能按该预设绑定的主代理那套面走。
